...
namespace CS6510_VirtualMachine_SJB.Memory
{
    ...
    public class Execute
    {
        ...
        public static void executePageProgram(VirtualMachine VM, int PID)
        {
            ...
            for (int i = VM.fp.runningQueue[PID].startPC; i < VM.fp.runningQueue[PID].endPC - 18; i += 6)
            {
                VM.PC = i;
                int[] instruction = VM.cpu.getNextInstruction(VM);
                int destination;
                int source_1;
                int source_2;
                int tempPID = 0;
                switch (instruction[0])
                {
                    case (byte)AssemblyInstruction.ADD:
                        ...
                        source_1 = instruction[2];
                        source_2 = instruction[3];
